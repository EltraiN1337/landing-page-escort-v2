...
{
	wp_enqueue_script('uncode-ot-admin', get_template_directory_uri() . '/core/assets/js/min/ot-admin-extended.min.js', array('ot-admin-js'), UNCODE_VERSION, false);
...
function uncode_load_admin_script() {
	...
	$site_parameters = array(
		...
		'modal_buttons'     => array(
			'cancel'  => esc_html__( 'Cancel', 'uncode' ),
			'confirm' => esc_html__( 'Confirm', 'uncode' ),
		),
		'media_cats'        => array(
...
function uncode_init_admin_css()
{
	...
	if (isset($fonts['font_stack']) && $fonts['font_stack'] !== '[]') {
		...
		if (@file_exists(trailingslashit($upload_dir['basedir']).'uncode-fonts/uncodefont.css')) wp_enqueue_style('uf-font-squirrel', $upload_dir_url .'/uncode-fonts/uncodefont.css', false ,$resources_version);
	}
}
...
if ( $max_input_vars < 3000 ) {
	...
	if (is_admin() && $pagenow === 'admin.php' && $_GET['page'] === 'uncode-options') {
	    ...
	    <div class="notice error is-dismissible">
	      ...
	      <p>
	      	<?php echo sprintf( wp_kses(__( 'Before saving the theme options you need to address an issue marked on the <a href="%s">Welcome Screen</a>.', 'uncode' ), array( 'a' => array( 'href' => array(),'target' => array() ) ) ), admin_url('admin.php?page=uncode-system-status') ); ?>
	      </p>
	    </div>
	    <?php
		}
...
function uncode_core_update_nag() {
	if (!is_admin() || !file_exists(WP_PLUGIN_DIR . '/uncode-core/uncode-core.php')) return false;
	...
	if (isset($GLOBALS['tgmpa']->plugins['uncode-core']['version']) && isset($uncode_core['Version'])) {
		if ( version_compare( $GLOBALS['tgmpa']->plugins['uncode-core']['version'], $uncode_core['Version'], '>' ) ) {
			?>
			<div class="notice error is-dismissible">
				<p><?php echo esc_html__( 'You need to update the Uncode Core plugin.', 'uncode' ); ?></p>
			</div>
			<?php
		}
	}
...
function uncode_user_add_meta_featured_image( $user ) {
    ...
    <table class="form-table">
		<tr id="user-featured-image" class="user-featured-image-wrap">
			<th>
				<label for="user_uncode_meta[term_media]"><?php esc_html_e( 'Featured media', 'uncode' ); ?></label>
			</th>
			<td>
				<div class="format-setting-inner">
					<div class="option-tree-ui-upload-parent">
					...
					<?php
						if ( isset($user_uncode_meta['term_media']) && wp_attachment_is_image( $user_uncode_meta['term_media'] ) ) {
							...
							if (isset($post->post_mime_type) && $post->post_mime_type === 'oembed/svg') echo '<div class="option-tree-ui-image-wrap">' . $post->post_content . '</div>';
								...
								echo '<div class="option-tree-ui-image-wrap"><img src="' . esc_url( $user_uncode_meta['term_media'] ) . '" alt="" /></div>';
							else echo '<div class="option-tree-ui-image-wrap"><div class="option-tree-ui-image-wrap"><div class="oembed" onload="alert(\'load\');"><span class="spinner" style="display: block; float: left;"></span></div><div class="oembed_code" style="display: none;">' . esc_url( $user_uncode_meta['term_media'] ) . '</div></div></div>';
							echo '<a href="#" class="option-tree-ui-remove-media option-tree-ui-button button button-secondary light" title="' . esc_html__( 'Remove Media', 'uncode' ) . '"><span class="icon fa fa-minus2"></span>' . esc_html__( 'Remove Media', 'uncode' ) . '</a>';
							echo '</div>';
						}
					?>
...
add_action( 'admin_init', 'uncode_check_for_previous_consistency' );
if ( !function_exists('uncode_check_for_previous_consistency') ):
/**
 * Check if author module exists from previous version, otherwise set default values.
 * Check a new version has installed.
 * @since Uncode 1.6.1
 */
function uncode_check_for_previous_consistency(){
	...
	if ( !get_option('uncode_latest_version') || version_compare( get_option('uncode_latest_version'), UNCODE_VERSION, '<' ) ) {
		update_option('uncode_latest_version', UNCODE_VERSION);
		do_action('uncode_upgraded');
	}
}
endif;//uncode_check_for_previous_consistency

add_action('uncode_upgraded', 'uncode_create_dynamic_css');

add_filter( 'woocommerce_template_path', 'uncode_woo_bw_compatibility' );
if ( !function_exists('uncode_woo_bw_compatibility') ):
/**
 * Filter WooCommerce path for backwars compatibility.
 * @since Uncode 1.5.0
 */
function uncode_woo_bw_compatibility($woocommerce) {
	$WC_vers = uncode_get_WC_version();
	if ( version_compare( $WC_vers, '3.0', '<' ) )
		$woocommerce = 'woocommerce/2.6.x/';

	return $woocommerce;
}
endif;//uncode_woo_bw_compatibility
...
function uncode_change_menu_cap( $menu_ord ) {
    ...
    $array_first = array_slice($submenu['uncode-system-status'], 0, ($options_pos));
    $array_second = array_slice($submenu['uncode-system-status'], ($options_pos));
    ...
    $submenu['uncode-system-status'] = $array_first;

    return $menu_ord;
}
endif;//uncode_change_menu_cap

add_action( 'admin_menu', 'uncode_related_post_submenu', 100 );
if ( !function_exists('uncode_related_post_submenu') ):
/**
 * @since Uncode 1.5.0
 */
function uncode_related_post_submenu() {
	if ( is_plugin_active( 'related-posts-for-wp/related-posts-for-wp.php' ) && class_exists('RP4WP_Hook_Settings_Page') ) {
		...
		add_submenu_page( 'uncode-system-status', 'UNCODE', esc_html__( 'Related Posts', 'related-posts-for-wp' ), 'manage_options', 'rp4wp', array(
			'Uncode_RP4WP_Hook_Settings_Page',
			'screen'
		), 'uncode-system-status', 'uncode_welcome_page' );

	}
}
endif;//uncode_related_post_submenu

add_action( 'admin_menu', 'uncode_VC_submenu_pages', 100 );
if ( !function_exists('uncode_VC_submenu_pages') ):
/**
 * @since Uncode 1.5.0
 */
function uncode_VC_submenu_pages() {
	remove_submenu_page( 'vc-general', 'edit.php?post_type=vc_grid_item' );
}
endif;//uncode_VC_submenu_pages

add_action( 'init', 'uncode_VC_deregister_cpt', 100 );
if ( !function_exists('uncode_VC_deregister_cpt') ):
/**
 * @since Uncode 1.5.0
 */
function uncode_VC_deregister_cpt() {
	if ( function_exists('unregister_post_type'))//WP 4.5+
		unregister_post_type( 'vc_grid_item' );
}
endif;//uncode_VC_deregister_cpt
...
?>
    <div class="notice error is-dismissible" id="uncode_vc_admin_notice">
        <p><?php printf( wp_kses_post( __( 'Please activate page builder for Content Block in Visual Composer <a href="%1s">Role Manager</a>. More info on the documentation, <a href="%2s" target="_blank">click here</a>', 'uncode' ) ), esc_url( admin_url( 'admin.php?page=vc-roles' ) ), esc_url( 'https://support.undsgn.com/hc/en-us/articles/214006125' ) ); ?></p>
    </div>
<?php
    }
